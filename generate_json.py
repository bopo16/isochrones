...
def build_json_query(location, arrival_time, times):
    ...
    # Loop through each travel_time and create an arrival_search item
    for time in times:
        # Create a copy of the base arrival_search to modify
        arrival_search = base_arrival_search.copy()
        arrival_search["id"] = f"isochrone-{time}"  # Optional: unique ID for each
        arrival_search["travel_time"] = time*60
